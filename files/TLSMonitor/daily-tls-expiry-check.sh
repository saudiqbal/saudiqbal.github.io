...
logger -t TLSMonitor -p warning "[TLS CHECKER] Expiry status change detected in $domainname, SQLite updated"
else
logger -t TLSMonitor -p warning "[TLS CHECKER] No Expiry status change detected in expired $domainname, SQLite update skipped"
fi
# Send notification only once
if [ "${statusvalue}" = "OK" ]; then
# API Call or Email
logger -t TLSMonitor -p warning "[TLS CHECKER] Calling notification API"
#mail -s "Certificate expires on $(date -d @"$expirationdate" '+%Y-%m-%d %H:%M:%S')" -r "from@example.com" "to@example.com" <<< "[$date] [TLS CHECKER] Certificate for $domainname expires in less than $DAYS days, on $(date -d @"$expirationdate" '+%Y-%m-%d %H:%M:%S')"
#curl -sSG "http://example.com/api.php?token=password" --data-urlencode "title=[TLS CHECKER] Certificate expires on $(date -d @"$expirationdate" '+%Y-%m-%d %H:%M:%S')" --data-urlencode "message=[$date] Certificate for $domainname expires in less than $DAYS days, on $(date -d @"$expirationdate" '+%Y-%m-%d %H:%M:%S')"
fi
else
...
logger -t TLSMonitor -p warning "[TLS CHECKER] TimeStamp change detected in $domainname, Old: $(date -d @"$timestampvalue" '+%Y-%m-%d %H:%M:%S') / New: $(date -d @"$expirationdate" '+%Y-%m-%d %H:%M:%S'), SQLite updated"
else
...
# API Call or Email
#mail -s "Certificate expires(ed) today" -r "from@example.com" "to@example.com" <<< "[$date] [TLS CHECKER] Certificate for $domainname expires(ed) today"
#curl -sSG "http://example.com/api.php?token=password" --data-urlencode "title=[TLS CHECKER] Certificate expires(ed) today" --data-urlencode "message=[$date] Certificate for $domainname expires(ed) today"
fi
done
#unset IFS
